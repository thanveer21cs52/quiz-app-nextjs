  ...
  return (
    <div className=" bg-quizbg text-quizbutton p-6 flex flex-col space-y-4 justify-center items-start h-fit w-2/4">
      <div className="flex w-full justify-between">
        <div className="text-quiztimer flex justify-start ">
          {seconds == 0 && "Times Up Go Next"}
        </div>
        <div className="text-quiztimer flex justify-end">
          Timer: 00:{seconds != 10 && 0}
          {seconds}
        </div>
      </div>
          ...
          return (
            <label
              ...
              className={`flex ${
                ...
                correctanswer.id == youranswer.id &&
                issubmit &&
                optionid == youranswer.id &&
                "bg-green-400"
              } ${
                optionid == youranswer.id &&
                issubmit &&
                optionid != correctanswer.id &&
                "bg-red-400"
              } p-2 border-quizbutton border-2 space-x-2 shadow-black ${
                !issubmit && "hover:bg-quizhover"
              } ${issubmit && "text-black/30"} `}
            >
              <input
                type="radio"
                id={optionid}
      ...
      <div className="flex justify-center items-center w-full  mt-5">
        <button
          className="text-sm bg-quizbutton w-3/4 text-gray-50  py-2 rounded-sm cursor-pointer hover:bg-amber-800"
          type="button"
          onClick={() => {
